...
void print_info(void)
{
  ...
  int max_column_lens[] = {
      ...
      [PRIORITY_RATIO] 5 + min_space_between_words,
      [ARRIVAL_TIME_RATIO] 5 + min_space_between_words,
      [EXECUTED_CYCLES_RATIO] 5 + min_space_between_words,
  ...
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
  {
    ...
    char rank_str[30];
    float_to_string(rank, rank_str, RATIO_PRECISION);
